...
public record GetMostRepeatedActorQuery(Guid UserId) : IRequest<ActorCountDto?>;
public class GetMostRepeatedActorQueryHandler : IRequestHandler<GetMostRepeatedActorQuery, ActorCountDto?>
{
    ...
    {
        return await _queries.GetMostRepeatedActorAsync(request.UserId, ct);
    }
}

// 7. Identify the director with the most movies in the watched history
public record GetMostWatchedDirectorQuery(Guid UserId) : IRequest<DirectorCountDto?>;
public class GetMostWatchedDirectorQueryHandler : IRequestHandler<GetMostWatchedDirectorQuery, DirectorCountDto?>
{
    ...
    {
        return await _queries.GetMostWatchedDirectorAsync(request.UserId, ct);
    }
}
...
public record GetDirectorRankingByRatingQuery(Guid UserId) : IRequest<IEnumerable<DirectorLeaderboardDto>>;
public class GetDirectorRankingByRatingQueryHandler : IRequestHandler<GetDirectorRankingByRatingQuery, IEnumerable<DirectorLeaderboardDto>>
{
    ...
    {
        return await _queries.GetDirectorRankingByRatingAsync(request.UserId, ct);
    }
}

// 10. Calculation of total time invested watching movies of a specific director or genre
public record GetTotalTimeByDirectorOrGenreQuery(Guid UserId, string FilterType, string FilterName) : IRequest<TimeInvestedDto?>;
public class GetTotalTimeByDirectorOrGenreQueryHandler : IRequestHandler<GetTotalTimeByDirectorOrGenreQuery, TimeInvestedDto?>
{
    ...
    {
        return await _queries.GetTotalTimeByDirectorOrGenreAsync(request.UserId, request.FilterType, request.FilterName, ct);
    }
}
